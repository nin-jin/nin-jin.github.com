...
,   function( nodeRoot ){
        ...
        function( ){
            nodeContent.clear()
            var chunks= $doc().location.hash.substring( 1 ).split( ';' )
            for( var i= 0; i < chunks.length; ++i ){
                var pair= chunks[i].split( '=' )
                if( pair.length < 2 ) continue
        ...
        ,   function( ){
                var chunks= []
                var headerList= nodeContent.descList( 'h1' )
                for( var i= 0; i < headerList.length(); ++i ){
                    var header= headerList.get( i )
                    var source= $String( header.text() ).trim( /[\r\n]/ ).replace( /    /, '\t' ).$
                    chunks.push( 'h1=' + encodeURIComponent( source ) )
                }
                var testList= nodeContent.descList( 'wc:js-test_source' )
                for( var i= 0; i < testList.length(); ++i ){
                    var test= testList.get( i )
                    var source= $String( test.text() ).trim( /[\r\n]/ ).replace( /    /, '\t' ).$
                    chunks.push( 'wc:js-test=' + encodeURIComponent( source ) )
                }
        ...
        var onClone=
        nodeRoot.listen( '$jam.$eventClone', save )
...
;// snippet/-mix+doc/compiled.vml.js
