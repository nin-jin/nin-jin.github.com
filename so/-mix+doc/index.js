...
var openEncoded= $jam.CE.escape( open )
var closeEncoded= $jam.CE.escape( close )
var Selector= arg.Selector || arg.encoder && factory.Selector( arg.encoder ) || factory.Selector()
var parse= $jam.CG( new function(){
...
$jam.BQ
(   $jam.CU( function( ){
return $jam.BG( factory.Selector( $jam.CD() ) )
})
,   function( proc ){
...
(   '$jam.CY'
,   $jam.CU( function(){
var poolNode= $jam.CU( function(){
var body= $jam.BJ().getElementsByTagName( 'body' )[ 0 ]
var pool= $jam.BJ().createElement( 'wc:BB:pool' )
...
var Template= $jam.CX({ Selector: Selector })
...
var lineParser= $jam.CE.build( '^((?:', oneIndent, ')*)(.*?)(?:', valSep, '(.*))?$' ).$
var parser=
function( str ){
...
$jam.CA().type( '$jam.DE' ).scream( event.target() )
}
$jam.BX( $jam.BJ().documentElement )
.listen( 'keyup', handler )
}
...
var ratio= parseFloat( nodeRoot.attr( 'wc:W' ) )
...
return new function( ){
nodeRoot= $jam.BX( nodeRoot )
var source= $jam.CL( nodeRoot.text() ).minimizeIndent().trim( /[\n\r]/ ).$
nodeRoot.clear()
var nodeResult=
$jam.BX.Element( 'wc:AK' )
.parent( nodeRoot )
var nodeSource0=
$jam.BX.Element( 'wc:AL' )
.parent( nodeRoot )
var nodeSource=
$jam.BX.parse( '<wc:K wc:L="sgml" />' )
.text( source )
.parent( nodeSource0 )
var exec= $jam.CY( function( ){
var source= $jam.CL( nodeSource.text() ).minimizeIndent().trim( /[\n\r]/ )
...
$jam.BO( 'wc:BC', function( nodeRoot ){
nodeRoot= $jam.BX( nodeRoot )
...
var message= $jam.BX.Element( 'wc:BD' )
$jam.BX.Element( 'wc:BE' ).text( user.display_name ).parent( message )
$jam.BX.Element( 'wc:BF' ).text( post.raw_message ).parent( message )
...
nodeRoot= $jam.BX( nodeRoot )
var nodeHeader=
$jam.BX.parse( '<wc:AR title="ctrl + enter" />' )
.tail( $jam.BX.parse( '<wc:AU>Run ►' ) )
.tail( $jam.BX.parse( '<wc:AT>inner (µs)' ) )
.tail( $jam.BX.parse( '<wc:AT>outer (µs)' ) )
nodeRoot.head( nodeHeader )
//var nodeControls= $jam.BX.Element( 'wc:AN' ).parent( nodeRoot )
//var nodeClone= $jam.BX.parse( '<wc:AO title="ctrl+shift+enter">clone' ).parent( nodeControls )
//var nodeDelete= $jam.BX.parse( '<wc:AP>delete' ).parent( nodeControls )
var refresh=
function( ){
var benchList= nodeRoot.childList( 'wc:AS' )
...
.clear()
var nodeSource=
$jam.BX.parse( '<wc:AV><wc:K wc:L="js">' + $jam.CC( source ) )
.parent( nodeRoot )
var nodeInner=
$jam.BX.parse( '<wc:AW class=" source=inner " />' )
.parent( nodeRoot )
var nodeOuter=
$jam.BX.parse( '<wc:AW class=" source=outer " />' )
.parent( nodeRoot )
nodeRoot.surround( $jam.BX.Fragment() ) // for chrome 12
var calc= $jam.CY( function( source ){
...
function( ){
var node=
$jam.BX.Element( 'wc:AS' )
.text( nodeSource.text() )
nodeRoot.prev( node )
}
var onCommit=
nodeRoot.listen( '$jam.CK', schedule )
var onClone=
nodeRoot.listen( '$jam.DD', clone )
return new function( ){
this.destroy=
function( ){
onCommit.sleep()
...
var source= $jam.CL( nodeRoot.text() ).minimizeIndent().trim( /[\n\r]/ ).$
nodeRoot.clear()
var nodeSource0= $jam.BX.Element( 'wc:AY' ).parent( nodeRoot )
var nodeSource= $jam.BX.parse( '<wc:K wc:L="js" />' ).text( source ).parent( nodeSource0 )
var nodeControls= $jam.BX.Element( 'wc:AN' ).parent( nodeRoot )
var nodeClone= $jam.BX.parse( '<wc:AO title="ctrl+shift+enter">clone' ).parent( nodeControls )
var nodeDelete= $jam.BX.parse( '<wc:AP>delete' ).parent( nodeControls )
var _test= {}
...
var run=
function( ){
var results= nodeRoot.childList( 'wc:AZ' )
...
run()
var onCommit=
nodeRoot.listen( '$jam.CK', run )
var onClone=
nodeRoot.listen( '$jam.DD', clone )
var onClone=
nodeRoot.listen( '$jam.DE', del )
var onCloneClick=
...
;$jam.CO( 'https://github.com/nin-jin/wc' )
;$jam.BO
(   'wc:BA'
,   function( nodeRoot ){
nodeRoot=
...
;$jam.CO( 'https://github.com/nin-jin/doc' )
