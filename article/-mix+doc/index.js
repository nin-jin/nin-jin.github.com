...
if(!( this instanceof $jam.BP )) return new $jam.BP( tagName, factory )
...
var node= ev.target
check4attach([ node ])
if( !$jam.BO.msie() && node.getElementsByTagName ) check4attach( node.getElementsByTagName( tagName ) )
}, false )
docEl.addEventListener( 'DOMNodeRemoved', function whenNodeRemoved( ev ){
var node= ev.target
check4detach([ node ])
if( !$jam.BO.msie() && node.getElementsByTagName ) check4detach( node.getElementsByTagName( tagName ) )
}, false )
...
if( name ) return $jam.BS[ name ] || str
...
if( superClass !== $jam.BQ ) return obj
...
proto.scream=
function( node ){
$jam.BZ( node ).dispatchEvent( this.$ )
return this
}
})
;$jam.BI
...
function( event ){
return handler.call( self.node(), $jam.CB( event ) )
}
return this
}
)
...
,   function( key ){
return $jam.BW({ splitterChunks: ' ' }).merge( this.$.className || '' ).get( key )
}
,   function( key, value ){
this.$.className= $jam.BW({ splitterChunks: ' ' }).merge( this.$.className ).put( key, value )
return this
}
)
proto.width=
...
//if( this.name() === 'br' ) return this;//this.prev( $jam.BY.Text( '\r\n' ) )
this.parent( null )
return this
}
proto.dissolveTree=
...
$jam.BR
(   function(){
return $jam.BY( this.$.firstChild )
}
,   function( node ){
this.$.insertBefore( $jam.BZ( node ), this.$.firstChild )
return this
}
)
proto.tail=
$jam.BR
(   function(){
return $jam.BY( this.$.lastChild )
}
,   function( node ){
this.$.appendChild( $jam.BZ( node ) )
return this
}
)
proto.next=
$jam.BR
(   function(){
return $jam.BY( this.$.nextSibling )
}
,   function( node ){
var parent= this.$.parentNode
var next= this.$.nextSibling
parent.insertBefore( $jam.BZ( node ), next ) 
return this
}   
)
proto.delve=
...
$jam.BR
(   function(){
return $jam.BY( this.$.previousSibling )
}
,   function( node ){
node= $jam.BZ( node )
var parent= this.$.parentNode
...
proto.clone=
function( ){
return $jam.BY( this.$.cloneNode( false ) )
}
proto.cloneTree=
function( ){
return $jam.BY( this.$.cloneNode( true ) )
}
proto.listen=
...
$jam.BY( el ).state( 'target', isTarget )
}
)
;$jam.BI
(   '$jam.CD'
...
proc.importStylesheet( $jam.BZ( stylesheet ) )
var doc= proc.transformToDocument( this.$ )
return $jam.CJ( doc )
}
,   'ms': function( stylesheet ){
var text= this.$.transformNode( $jam.BZ( stylesheet ) )
return $jam.CJ.parse( text )
}
})
klass.parse=
$jam.BO.xmlModel.select(
...
$jam.BR
(   function( ){
return $jam.BY( this.$.cloneContents() ).toString()
}
,   function( html ){
var node= html ? $jam.BY.parse( html ).$ : $jam.BY.Text( '' ).$
this.replace( node )
return this
}
)
...
var current= $jam.BY( this.$.startContainer )
if( this.$.startOffset ){
var temp= current.$.childNodes[ this.$.startOffset - 1 ]
if( temp ){
current= $jam.BY( temp ).follow()
} else {
...
var range= $jam.CM().aimNode( current )
...
proto.clone=
function( ){
return $jam.CM( this.$.cloneRange() )
}
proto.aimNodeContent=
function( node ){
this.$.selectNodeContents( $jam.BZ( node ) )
return this
}
proto.aimNode=
function( node ){
this.$.selectNode( $jam.BZ( node ) )
return this
}
})
)
...
if( i % 2 ) chunk= $jam.CO.escape( chunk )
str+= chunk
}
return $jam.CO( str )
}
...
var regexp= $jam.CO( lexems[ name ] )
nameList.push( name )
regexpList.push( regexp.source() )
sizeList.push( regexp.count() )
}
var regexp= RegExp( '([\\s\\S]*?)(?:((' + regexpList.join( ')|(' ) + '))|($\n?))', 'gm' )
var count= $jam.CO(regexp).count()
return $jam.BQ( function( klass, proto ){
proto.constructor=
function( str ){
...
this.$.clock=
$jam.CG()
.handler( $jam.CF( this ).method( 'run' ) )
return this
}
proto.latency=
$jam.BR
...
var openEncoded= $jam.CO.escape( open )
var closeEncoded= $jam.CO.escape( close )
var Selector= arg.Selector || arg.encoder && factory.Selector( arg.encoder ) || factory.Selector()
var parse= $jam.CS( new function(){
...
$jam.BR
(   $jam.CN( function( ){
return $jam.BG( factory.Selector( $jam.CR() ) )
})
,   function( proc ){
...
(   '$jam.CV'
,   $jam.CN( function(){
var poolNode= $jam.CN( function(){
var body= $jam.BJ().getElementsByTagName( 'body' )[ 0 ]
var pool= $jam.BJ().createElement( 'wc:AW:pool' )
...
var lineParser= $jam.CO.build( '^((?:', oneIndent, ')*)(.*?)(?:', valSep, '(.*))?$' ).$
var parser=
function( str ){
...
$jam.CB().type( '$jam.DB' ).scream( event.target() )
}
$jam.BY( $jam.BJ().documentElement )
...
$jam.CB().type( '$jam.DC' ).scream( event.target() )
}
$jam.BY( $jam.BJ().documentElement )
...
$jam.CB().type( '$jam.DD' ).scream( event.target() )
}
$jam.BY( $jam.BJ().documentElement )
.listen( 'keyup', handler )
}
...
(   50
,   function( target ){
$jam.CB().type( '$jam.DE' ).scream( target )
}
)
var handler=
function( event ){
...
var docEl= $jam.BY( $jam.BJ().documentElement )
...
lastURI= newURI
$jam.CB().type( '$jam.DG' ).scream( $jam.BJ() )
}
...
this[ /(\/\*[\s\S]*?\*\/)/.source ]=
$jam.CR( $lang.text, css.remark )
this[ /(\*|(?:\\[\s\S]|[\w-])+)/.source ]=
$jam.CR( $lang.text, css.tag )
this[ /(#(?:\\[\s\S]|[\w-])+)/.source ]=
$jam.CR( $lang.text, css.id )
this[ /(\.(?:\\[\s\S]|[\w-])+)/.source ]=
$jam.CR( $lang.text, css.klass )
this[ /(::?(?:\\[\s\S]|[\w-])+)/.source ]=
$jam.CR( $lang.text, css.pseudo )
this[ /\{([\s\S]+?)\}/.source ]=
new function( ){
...
$jam.CR( $lang.text, css.remark )
this[ /([\w-]+\s*:)/.source  ]=
$jam.CR( $lang.text, css.property )
this[ /([^:]+?(?:;|$))/.source ]=
$jam.CR( $lang.text, css.value )
})
return css
}
;$lang.pcre=
...
$lang.Parser( new function(){
this[ /(\/\*[\s\S]*?\*\/)/.source ]=
$jam.CR( $lang.text, js.remark )
this[ /(\/\/[^\n]*)/.source ]=
$jam.CR( $lang.text, js.remark )
this[ /('(?:[^\n'\\]*(?:\\\\|\\[^\\]))*[^\n'\\]*')/.source ]=
$jam.CR( $lang.text, js.string )
this[ /("(?:[^\n"\\]*(?:\\\\|\\[^\\]))*[^\n"\\]*")/.source ]=
$jam.CR( $lang.text, js.string )
this[ /(\/(?:[^\n\/\\]*(?:\\\\|\\[^\\]))*[^\n\/\\]*\/[mig]*)/.source ]=
$jam.CR( $lang.pcre, js.regexp )
this[ /\b(_[\w$]*)\b/.source ]=
$jam.CR( $lang.text, js.internal )
this[ /(\$[\w$]*)(?![\w$])/.source ]=
$jam.CR( $lang.text, js.external )
...
$jam.CR( $lang.text, js.keyword )
this[ /((?:\d*\.)?\d(?:[eE])?)/.source ]=
$jam.CR( $lang.text, js.number )
this[ /([(){}\[\]])/.source ]=
$jam.CR( $lang.text, js.bracket )
this[ /(\+{1,2}|-{1,2}|\*|\/|&{1,2}|\|{1,2}|={1,2}|%|\^|!)/.source ]=
$jam.CR( $lang.text, js.operator )
})
return js
}
;$lang.sgml=
...
$lang.Parser( new function(){
this[ /(<!--[\s\S]*?-->)/.source ]=
$jam.CR( $lang.text, sgml.comment )
this[ /(<![\s\S]*?>)/.source ]=
$jam.CR( $lang.text, sgml.decl )
...
return new function( ){
nodeRoot= $jam.BY( nodeRoot )
var source= $jam.CI( nodeRoot.text() ).minimizeIndent().trim( /[\n\r]/ ).$
nodeRoot.clear()
var nodeResult=
$jam.BY.Element( 'wc:S' )
.parent( nodeRoot )
...
this[ /('(?:[^\n'\\]*(?:\\\\|\\[^\\]))*[^\n'\\]*')/.source ]=
this[ /("(?:[^\n"\\]*(?:\\\\|\\[^\\]))*[^\n"\\]*")/.source ]=
$jam.CR( $lang.text, php.string )
this[ /((?:\d*\.)?\d(?:[eE])?)/.source ]=
$jam.CR( $lang.text, php.number )
})
return php
}
;$jam.BI
...
var embed= md.embed( '<wc:M wc:AX=".75"><iframe class="wc_lang_md_embed-object" src="' + uri + '" allowfullscreen></iframe></wc:M>' )
return href + embed
}
// image
...
$jam.BP( 'wc:BA', function( nodeRoot ){
nodeRoot= $jam.BY( nodeRoot )
var script= $jam.BY.Element( 'script' ).attr( 'src', '//nin-jin.disqus.com/thread.js?url=' + $jam.DI( '//' + document.location.host + document.location.pathname ) )
...
var message= $jam.BY.Element( 'wc:BB' )
$jam.BY.Element( 'wc:BC' ).text( user.display_name ).parent( message )
$jam.BY.Element( 'wc:BD' ).text( post.raw_message ).parent( message )
...
nodeRoot.clear()
var nodeSource= $jam.BY.parse( '<div class=" wc_editor_content " />' )
.html( source )
.parent( nodeRoot )
var sourceLast= ''
var update= $jam.CW( 50, function( ){
//var source= $jam.CI( nodeSource.text() ).replace( /\n?\r?$/, '\n' ).$
...
.replace( /\n/g, '<br/>' )
.$
var nodeRange= $jam.CM().aimNodeContent( nodeSource )
var startPoint= $jam.CM().collapse2start()
//console.log(nodeRange.html())
var endPoint= $jam.CM().collapse2end()
...
//if( source.charAt( source.length -1 ) !== '\n' ) nodeSource.tail( $jam.BY.Text( '\n' ) )
//if( !source ) $jam.CM().aimNode( nodeSource.head() ).collapse2end().select()
//if( nodeSource.tail() && nodeSource.tail().name() !== 'br' ) nodeSource.tail( $jam.BY.Element( 'br' ) )
} )
var onEdit=
...
event.defaultBehavior( false )
$jam.CM().html( '<br/>' ).collapse2end().select()
})
var onAltSymbol=
...
var symbol= symbolSet[ $jam.CA( event.keyCode() ) ]
if( !symbol ) return
event.defaultBehavior( false )
$jam.CM().text( symbol ).collapse2end().select()
})
//var onBackspace=
//nodeRoot.listen( 'keydown', function( event ){
//    event= $jam.CB( event )
...
//    if( event.keyAccel() ) return
//    event.defaultBehavior( false )
//    var fullRange= $jam.CM().aimNodeContent( nodeSource )
//    var newOffset= fullRange.clone().equalize( 'end2start', $jam.CM() ).text().length - 1
//    if( newOffset < 0 ) newOffset= 0
//    var range= fullRange.clone().move( newOffset ).equalize( 'end2end', $jam.CM() )
//    range.dropContents()
//})
var onTab=
nodeRoot.listen( 'keydown', function( event ){
...
if( event.keyAccel() ) return
event.defaultBehavior( false )
$jam.CM().text( '    ' ).collapse2end().select()
})
var onLeave=
nodeSource.listen( 'blur', function( event ){
$jam.CB().type( '$jam.DC' ).scream( nodeRoot )
})
var onActivate=
nodeRoot.listen( 'mousedown', function( event ){
event= $jam.CB( event )
if( !event.keyMeta() ) return
nodeRoot.attr( 'wc:V', true )
...
nodeRoot= $jam.BY( nodeRoot )
var nodeHeader=
$jam.BY.parse( '<wc:AD title="ctrl + enter" />' )
.tail( $jam.BY.parse( '<wc:AG>Run ►' ) )
.tail( $jam.BY.parse( '<wc:AF>inner (µs)' ) )
.tail( $jam.BY.parse( '<wc:AF>outer (µs)' ) )
nodeRoot.head( nodeHeader )
//var nodeControls= $jam.BY.Element( 'wc:Z' ).parent( nodeRoot )
//var nodeClone= $jam.BY.parse( '<wc:AA title="ctrl+shift+enter">clone' ).parent( nodeControls )
//var nodeDelete= $jam.BY.parse( '<wc:AB>delete' ).parent( nodeControls )
var refresh=
function( ){
var benchList= nodeRoot.childList( 'wc:AE' )
...
.clear()
var nodeSource=
$jam.BY.parse( '<wc:AH><wc:U wc:AY="js">' + $jam.CL( source ) )
.parent( nodeRoot )
var nodeInner=
$jam.BY.parse( '<wc:AI class=" source=inner " />' )
.parent( nodeRoot )
var nodeOuter=
$jam.BY.parse( '<wc:AI class=" source=outer " />' )
.parent( nodeRoot )
nodeRoot.surround( $jam.BY.Fragment() ) // for chrome 12
...
var source= $jam.CI( nodeRoot.text() ).minimizeIndent().trim( /[\n\r]/ ).$
nodeRoot.clear()
var nodeSource0= $jam.BY.Element( 'wc:AK' ).parent( nodeRoot )
var nodeSource= $jam.BY.parse( '<wc:U wc:AY="js" />' ).text( source ).parent( nodeSource0 )
var nodeControls= $jam.BY.Element( 'wc:Z' ).parent( nodeRoot )
var nodeClone= $jam.BY.parse( '<wc:AA title="ctrl+shift+enter">clone' ).parent( nodeControls )
var nodeDelete= $jam.BY.parse( '<wc:AB>delete' ).parent( nodeControls )
var _test= {}
...
print( $jam.BV( val ) + ': ' + val )
}
var run=
function( ){
var results= nodeRoot.childList( 'wc:AL' )
...
var textLast= $jam.BU( nodeRoot.html() )
function save( ){
var text= $jam.BU( nodeRoot.html() )
...
;$jam.BM( 'https://github.com/nin-jin/wc' )
;$jam.BP
(   'wc:AR'
,   function( nodeRoot ){
nodeRoot=
...
if( !nodeFrame ) nodeFrame= $jam.BY.Element( 'iframe' ).parent( nodeRoot )
...
;$jam.BM( 'https://github.com/nin-jin/doc' )
